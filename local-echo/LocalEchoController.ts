...
export default class LocalEchoController implements ITerminalAddon {
   ...
   */
  removeAutocompleteHandler(fn: Function) {
   ...
   */
  printWide(items: any, padding = 2) {
    if (items.length == 0) return this.println("");

    // Compute item sizes and matrix row/cols
    const itemWidth =
      items.reduce((width: number, item: any) => Math.max(width, item.length), 0) + padding;
   ...
   */
  printAndRestartPrompt(callback: Function) {
